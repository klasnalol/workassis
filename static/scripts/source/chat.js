...
// On page load, populate microphone dropdown
$(document).ready(() => {
  populateMicrophones();
});
...
async function populateMicrophones() {
  try {
    ...
    $("#mic-select-chat").html(
      '<option value="">No microphone available</option>',
    );
    $("#record-voice-btn").prop("disabled", true);
    alert(
      "Could not access microphones. Check your browser settings and permissions.",
    );
  }
}
...
$("#record-voice-btn").click(async () => {
  ...
  try {
    ...
    chatModule.mediaRecorderVoice.onstop = () => {
      ...
      // Show bot thinking while server processes the voice
      showBotThinking();

      // Send voice data via AJAX
      $.ajax({
        url: "/chat_voice",
        type: "POST",
        data: formData,
